...
public class Provider {
    public static void main(String[] args) throws Exception {
        String providerId = "provider" + Math.abs(new Random().nextInt() % 100);
...
//        channel.exchangeDeclare(EXCHANGE_REQUEST_NAME, BuiltinExchangeType.TOPIC, false);
        ...
        bindTopics(QUEUE_NAME, EXCHANGE_NAME, channel);
        ...
        Consumer consumer = new DefaultConsumer(channel) {
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                ...
                if (!sender.equals("all")) { // nie admin wiec trzeba odpowiedziec
                    String key = sender + "." + providerId + "_" + (counter++) + ".response";
                    String responseMessage = "Response to " + sender + " from " + providerId +  " for " + splitKey[1] + " request";
                    channel.basicPublish(EXCHANGE_NAME, key,  null, responseMessage.getBytes());
